...
public class Slf4jSpyLogDelegator implements SpyLogDelegator
{
	...
	private final Logger connectionTxLogger = LoggerFactory.getLogger("jdbc.transcation");
		...
		String sql, long execTime)
	{
		...
		else
		{
			...
			{
				sqlTimingLogger.error(getDebugInfo() + nl + spyNo + ". {FAILED after " + execTime + " msec} " + sql, e);
			}
			else
			{
				sqlTimingLogger.error(header + " FAILED! {FAILED after " + execTime + " msec} " + sql, e);
			}
		}
	}
		...
		String sql, boolean debugInfo)
	{
		...
		out.append("{executed in ").append(execTime).append(" msec} ");
		out.append(sql);
	...
	@Override
	public void connectionTranscation(Spy spy, String methodCall, String msg) {
		if(connectionTxLogger.isInfoEnabled()){
			connectionTxLogger.info(spy.getConnectionNumber() + ". " + methodCall + "-" + msg);
		}
	}
}
